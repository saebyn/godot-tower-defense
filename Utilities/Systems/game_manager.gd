...
func hide_dialog():
    MyLogger.debug("GameManager", "Hiding dialog...")
    Dialogic.Text.hide_textbox()


func show_dialog():
    MyLogger.debug("GameManager", "Showing dialog...")
    Dialogic.Text.show_textbox()


func wait_for_enemy_appeared() -> void:
    MyLogger.debug("GameManager", "Waiting for enemy_appeared signal...")
    var main_scene = get_tree().current_scene
    if main_scene and main_scene.has_signal("enemy_appeared"):
        await main_scene.enemy_appeared
        MyLogger.debug("GameManager", "Received enemy_appeared signal.")
    else:
        MyLogger.warn("GameManager", "Current scene does not have enemy_appeared signal.")


func wait_for_enemy_attacked() -> void:
    MyLogger.debug("GameManager", "Waiting for enemy_attacked signal...")
    var main_scene = get_tree().current_scene
    if main_scene and main_scene.has_signal("enemy_attacked"):
        await main_scene.enemy_attacked
        MyLogger.debug("GameManager", "Received enemy_attacked signal.")
    else:
        MyLogger.warn("GameManager", "Current scene does not have enemy_attacked signal.")


func wait_for_achievement_unlocked(achievement_id: String) -> void:
    MyLogger.debug("GameManager", "Waiting for achievement_unlocked signal...")

    # Check if player has already unlocked the achievement to avoid waiting indefinitely
    if AchievementManager.is_achievement_unlocked(achievement_id):
        MyLogger.debug("GameManager", "Achievement %s is already unlocked." % achievement_id)
        return

    while true:
        var achivement = await AchievementManager.achievement_unlocked
        if achivement.id == achievement_id:
            MyLogger.debug("GameManager", "Achievement unlocked: %s" % achievement_id)
            break
    MyLogger.debug("GameManager", "Received achievement_unlocked signal.")


func wait_for_tech_unlocked(tech_id: String) -> void:
    MyLogger.debug("GameManager", "Waiting for tech_unlocked signal...")

    # Check if tech is already unlocked to avoid waiting indefinitely
    if TechTreeManager.is_tech_unlocked(tech_id):
        MyLogger.debug("GameManager", "Tech %s is already unlocked." % tech_id)
        return

    while true:
        var tech = await TechTreeManager.tech_unlocked
        if tech.id == tech_id:
            MyLogger.debug("GameManager", "Tech unlocked: %s" % tech_id)
            break
    MyLogger.debug("GameManager", "Received tech_unlocked signal.")


func wait_for_wave_cleared() -> void:
    MyLogger.debug("GameManager", "Waiting for wave_changed signal...")
    await ScenarioManager.wave_changed
    MyLogger.debug("GameManager", "Received wave_changed signal.")


func wait_for_building_placed() -> void:
    MyLogger.debug("GameManager", "Waiting for building_placed signal...")
    var main_scene = get_tree().current_scene
    if main_scene and main_scene.has_signal("building_placed"):
        await main_scene.building_placed
        MyLogger.debug("GameManager", "Received building_placed signal.")
    else:
        MyLogger.warn("GameManager", "Current scene does not have building_placed signal.")