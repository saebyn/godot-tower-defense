extends Control
class_name UI_ObstacleTooltip

## Tooltip that displays obstacle stats and active buffs when hovering over obstacles
...
## Update tooltip content based on current obstacle
func update_tooltip_content() -> void:
	if not current_obstacle:
		return
	
	var info = current_obstacle.get_tooltip_info()
	...
	var sources = current_obstacle.get_active_buff_sources()
...
func _process(_delta: float) -> void:
	if visible and current_obstacle:
...
func _get_buff_type_name(buff_type: Entity_BuffObstacle.BuffType) -> String:
	match buff_type:
