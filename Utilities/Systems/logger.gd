...
enum LogLevel { DEBUG = 0, INFO = 1, WARN = 2, ERROR = 3, NONE = 4 }  # Disables all logging
...
var _log_level_colors: PackedStringArray = [
  "[color=gray]", "[color=lightblue]", "[color=yellow]", "[color=red]"
]
...
func _ready() -> void:
  ...
  info(
    "Logger",
    (
      "Logging framework initialized - Level: %s, Scopes: %s"
      % [_log_level_names[current_log_level], ", ".join(enabled_scopes)]
    )
  )
...
func _register_console_commands() -> void:
  _console_commands = {
    "log_level": _cmd_set_log_level, "log_scopes": _cmd_set_scopes, "log_info": _cmd_show_info
  }
...
func _log(level: LogLevel, scope: String, message: String) -> void:
  ...
  var timestamp = "%02d:%02d:%02d.%03d" % [time.hour, time.minute, time.second, milliseconds]

  # Format the log message
  var level_name = _log_level_names[level]
  var color_tag = _log_level_colors[level]
  var formatted_message = (
    "%s[%s] [%s] %s: %s[/color]" % [color_tag, timestamp, level_name, scope, message]
  )
