...
func _is_placement_valid(target_position: Vector3) -> bool:
  ...
  return (
    _is_within_navigation_region(target_position)
    and not _has_obstacle_collision(target_position)
    and _has_terrain_support(target_position)
    and _has_sufficient_clearance(target_position)
  )
...
func _is_within_navigation_region(target_position: Vector3) -> bool:
  ...
  return (
    local_pos.x >= min_bounds.x + border_margin
    and local_pos.x <= max_bounds.x - border_margin
    and local_pos.z >= min_bounds.z + border_margin
    and local_pos.z <= max_bounds.z - border_margin
  )
